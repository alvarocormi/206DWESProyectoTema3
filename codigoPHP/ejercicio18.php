...
<body>
	...
	<main>
		<div class="contenido">
			<h2>Ejercicio 18 </h2>
			<p>REALIZAR EL EJERCICIO ANTERIOR PERO CON FUNCIONES</p>
			<?php
			/**
			...
			$teatro[2][5] = "Persona 1";
			$teatro[5][10] = "Persona 2";
			$teatro[10][3] = "Persona 3";
			$teatro[15][8] = "Persona 4";
			$teatro[18][12] = "Persona 5";

			//Recorremos el array usando foreach
			foreach ($teatro as $rowIndex => $row) {
				foreach ($row as $colIndex => $value) {
					echo "Fila: $rowIndex, Columna: $colIndex, Valor: $value<br>";
				}
			}

			//Usando for
			//Mediante count() podremos contar las filas de un array
			$rows = count($teatro);
			for ($i = 0; $i < $rows; $i++) {
				$columns = count($teatro[$i]);
				for ($j = 0; $j < $columns; $j++) {
					echo "Fila: $i, Columna: $j, Valor: " . $teatro[$i][$j] . "<br>";
				}
			}
